...
class Make_test_pdf extends Controller
{
	...
	function index($id = '',$user_id = '')
	{
		...
		//get teamLeader information
		if($data['answered_test_row']){
			
			$user = new User();
			$data['teamLeader_row'] = $user->first('user_id',$data['answered_test_row']->user_id);
		}
		...
		?>
			...
			<?php if($row && $answered_test_row && $answered_test_row->submitted):?>
				...
				<div style="font-family: roboto;font-size: 18px;max-width: 1000px;margin: auto;">
					<table>
						<tr>
						 	<td colspan="4" style="font-size:20px;text-align: center;">Quality Check: <?=esc(ucwords($row->test))?></td>
						</tr>
						<tr>
						 	<th>Team: </th><td><?=$row->team->team?></td>
						 	<th>Team Leader: </th><td><?=$teamLeader_row->firstname?> <?=$teamLeader_row->lastname?></td>
					 	</tr>
						<tr>
						...
						<tr>
							...
							<td style="text-align: center;" colspan="4"><b>Quality Check Description:</b><br><?=esc($row->description)?></td>
						</tr>
					</table>
					...
					<br>
						...
						<div class="container-fluid text-center">
							<span><?=$percentage?>% Checked | <?=$marked_percentage?>% Supervised</span>
 						</div>

						<?php if($marked):?>
						<center>
							<?php $score_percentage = get_score_percentage($row->test_id,$user_id)?>
							<small style="font-size:20px">Satisfaction:<br></small> <div style="font-size: 60px;margin-top: -10px;"><?=$score_percentage?>%</div>
						</center>
						<?php endif;?>
