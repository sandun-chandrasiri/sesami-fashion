...
/**
 * Reuse_pdf controller
 */
class Reuse_pdf extends Controller
{
    ...
    function index($id = '',$user_id = '')
    {
        ...
        $html = file_get_contents(ROOT.'/Make_reuse_pdf/'.$id.'/'.$user_id);
        $mpdf->WriteHTML($html);
        
        //get user details
        $reuse = new Reuse_model();
        $row = $reuse->first('reuse_id',$reuse_id);

        $file_name = $folder.$row->reuse.'_reuse_details_'.date("Y-m-d_H_i_s",time()).'.pdf';
