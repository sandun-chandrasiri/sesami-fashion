...
class Single_test extends Controller
{
	...
	public function index($id = '')
	{
		...
		$page_tab = 'view';
		//'$student_scores' has been replaced by $teamLeader_scores
		$teamLeader_scores = false;
		if(isset($_GET['tab']) && $_GET['tab'] == "scores")
		{
			$page_tab = 'scores';

			$answered_test = new Answered_test();
			$teamLeader_scores = $answered_test->query("select * from answered_tests where test_id = :test_id && submitted = 1 && marked = 1 order by score desc",['test_id'=>$id]); 
		...
		$data['errors'] 	= $errors;
		$data['pager'] 		= $pager;
		$data['teamLeader_scores'] 		= $teamLeader_scores;
