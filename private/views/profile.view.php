	...
	<div class="container-fluid p-4 shadow mx-auto" style="max-width: 1000px;">
		...
		<div class="row">
			<div class="col-sm-4 col-md-3">
				...
				<br>
				<?php if(Auth::access('head') || (Auth::access('dataEntryClerk') && $row->rank == 'teamLeader')):?>
		...
		<div class="container-fluid">
			<ul class="nav nav-tabs">
			  ...
			  <?php if(Auth::access('manager') || Auth::i_own_content($row)):?>
				  <li class="nav-item">
				    <a class="nav-link <?=$page_tab=='teams' ? 'active':'';?>" href="<?=ROOT?>/profile/<?=$row->user_id?>?tab=teams">My Teams</a>
				  </li>

				  <li class="nav-item">
				    <a class="nav-link <?=$page_tab=='tests' ? 'active':'';?>" href="<?=ROOT?>/profile/<?=$row->user_id?>?tab=tests">Quality Checks</a>
				  </li>
		 	<?php endif;?>
			</ul>
				...
				switch ($page_tab) {
					...
					case 'teams':
						...
						if(Auth::access('manager') || Auth::i_own_content($row)){
							include(views_path('profile-tab-teams'));
						}else{
