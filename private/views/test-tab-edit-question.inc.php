...
<center><h5>Edit <?=$quest_type?> Quality Criteria</h5></center>
		  ...
		  <ul class="list-group list-group-flush choice-list">
			...
			<?php else:?>
				...
				<?php foreach($choices as $letter => $answer):?>
				    <li class="list-group-item">
				    	<?=$letter?> : <input type="text" class="form-control" name="choice<?=$num?>" placeholder="Type your expected input here." value="<?=$answer?>">
				    	<label style="cursor: pointer;">
				    		<input <?= $letter == $question->correct_answer ? 'checked' : '';?> type="radio" value="<?=$letter?>" name="correct_answer"> Expecting input should be this choice.
				    	</label>
				    </li>
				<?php $num++;?>
				<?php endforeach;?>
...
<?php else:?>
	Sorry that Quality Criteria was not found!
	<br>
	<a href="<?=ROOT?>/single_test/<?=$row->test_id?>">
		<button type="button" class="btn btn-primary"><i class="fa fa-chevron-left"></i>Back</button>
	</a>
